...
/* A unidade só existe como rota nos mercados em que opera. Restringir os
   params aqui (com dynamicParams desligado) faz /py/pre-moldados ser uma URL que não
...
export function generateStaticParams() {
  return marketsForPath("/pre-moldados").map((locale) => ({ locale }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "meta.pre-moldados" });
  return {
    title: t("title"),
    description: t("description"),
    alternates: buildAlternates(locale, "/pre-moldados"),
  };
}
...
export default async function PreMoldadosPage({ params }: Props) {
  ...
  return <UnitPage market={locale} unit="pre-moldados" />;
}

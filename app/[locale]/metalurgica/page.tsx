...
/* A unidade só existe como rota nos mercados em que opera. Restringir os
   params aqui (com dynamicParams desligado) faz /py/metalurgica ser uma URL que não
...
export function generateStaticParams() {
  return marketsForPath("/metalurgica").map((locale) => ({ locale }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "meta.metalurgica" });
  return {
    title: t("title"),
    description: t("description"),
    alternates: buildAlternates(locale, "/metalurgica"),
  };
}
...
export default async function MetalurgicaPage({ params }: Props) {
  ...
  return <UnitPage market={locale} unit="metalurgica" />;
}
